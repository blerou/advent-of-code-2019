...
After providing 1 to the only input instruction and passing all the tests, what diagnostic code does the program produce?
  ...
  type Input = Int
  type Op = (Mem, Int, Input) => Int

  def paramMode(op: Int, paramNum: Int): Int = op / math.pow(10, paramNum + 1).toInt % 10

  def addOp(memory: Mem, address: Int, input: Input): Int = {
